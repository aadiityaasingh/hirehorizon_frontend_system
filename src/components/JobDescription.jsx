...
const JobDescription = () => {
  ...
  const applyJobHandler = async () => {
    try {
      ...
      if(res.data.success){
        setIsApplied(true) //for updation of local state
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="flex items-center gap-2 mt-4">
            ...
            <Badge className="text-violet-500 font-bold" variant="ghost">
        ...
        onClick = {isApplied ? null : applyJobHandler}
          disabled={isApplied}
          className="`rounded-lg ${isApplied ? ' bg-gray-600 cursor-not-allowed' : 'bg-blue-500 hover:bg-blue-300 cursor-pointer'}`"
        >
